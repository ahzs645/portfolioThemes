/*
 * Vanilla port of ReactBits' LiquidEther (MIT License) — the exact WebGL fluid
 * component 1800benson.ca uses for its background. Faithfully reproduces the
 * Three.js simulation: BFECC advection, optional viscosity, a Poisson pressure
 * solver, an additive cursor force, an idle auto-driver, and a colour pass that
 * maps velocity magnitude to a palette gradient with velocity-based alpha over a
 * transparent background (so the page's dark colour shows through — that
 * compositing is what gives the dark wine-red look and soft glow).
 *
 * createLiquidEther(container, options) mounts into `container` and returns a
 * dispose function. Logic mirrors the original; React refs are replaced with
 * closure-local variables so it works outside React.
 */
...
`;
  ...
  class Advection extends ShaderPass {
    ...
    update({ dt: dt_, isBounce: isBounce_, BFECC: BFECC_ }) {
      this.uniforms.dt.value = dt_;
      this.line.visible = isBounce_;
      this.uniforms.isBFECC.value = BFECC_;
      super.update();
    }
  }
  ...
  class Viscous extends ShaderPass {
    ...
    update({ viscous: viscous_, iterations, dt: dt_ }) {
      let fbo_in;
      let fbo_out;
      this.uniforms.v.value = viscous_;
  ...
  // Push the source's runtime options into the live simulation.
